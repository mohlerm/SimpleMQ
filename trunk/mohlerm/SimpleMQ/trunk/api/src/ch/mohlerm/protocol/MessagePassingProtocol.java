...
public class MessagePassingProtocol {
    ...
    @Deprecated
    public static int answerBufferSize(Logger log) {
        ...
        try {
            ...
            log.error("Can not get answer size");
            e.printStackTrace();
        }
    ...
    @Deprecated
    public static int requestBufferSize(Logger log) {
        // setup dummy message to get max size of buffer
        ...
        try {
            ...
            log.error("Can not get request size");
            e.printStackTrace();
        }
        return byteOutputStream.toByteArray().length;
    }

    public static SerializableRequest parseRequest(byte[] data, int count, Logger log) {
        ...
        try {
            ...
            log.error("Can not serialize: EOF");
            e.printStackTrace();
        } catch (NullPointerException e) {
            log.error("Can not serialize: Nullpointer");
            e.printStackTrace();
        } catch (IOException e) {
            log.error("Can not serialize: IOException");
            e.printStackTrace();
        } catch (ClassNotFoundException e) {
            log.error("Can not serialise: ClassNotFound");
            e.printStackTrace();
        }
        if(request==null) {
            request = new SerializableRequest(RequestType.ERROR, 0,0,0,0, "Error when deserializing");
        }
        return request;
    }
    public static SerializableAnswer parseAnswer(byte[] data, int count, Logger log) {
        byte[] dataCopy = new byte[count];
        System.arraycopy(data, 0, dataCopy, 0, count);
        ObjectInputStream obj = null;
        ...
        if(answer==null) {
            answer = new SerializableAnswer(RequestType.ERROR,0,0,0, "Error when deserializing");
        }
        return answer;
    }
}

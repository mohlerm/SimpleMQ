...
for i in range(0,len(ans_rcv_miliseconds)):
    if ans_rcv_miliseconds[i] < timestamps[low_water_mark+1]:
        ...
        ans_rcv_response_time_value[low_water_mark] += ans_rcv_response[i]
#logging.info(throughput_values)
print("time;throughput;snd_responsetime;rcv_responsetime")
for i in range(0,len(timestamps)-3):
    print(str(timestamps[i])+";"+str(throughput_values[i])+";"+str(ans_snd_response_time_value[i])+";"+str(ans_rcv_response_time_value[i]))
